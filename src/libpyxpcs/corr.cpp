...
py::array_t<float> Multitau(py::array_t<int> valid_pixels, py::list rows, py::list values, py::dict config) 
{
    ...
    int g2_index = 106620;
    printf("G2 at index %d, %f")

    return py::array_t<float>(
        {return_dims, no_of_pixels, delays_per_level.size()},
        {no_of_pixels*delays_per_level.size()*sizeof(float), delays_per_level.size()*sizeof(float), sizeof(float)},
        result,
        free_when_done
    );
